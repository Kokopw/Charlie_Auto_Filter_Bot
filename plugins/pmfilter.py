...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        # Check if this is a PM (private message)
        if query.message.chat.type == enums.ChatType.PRIVATE:
            # Check verification if IS_VERIFY is True
            if IS_VERIFY:
                user_id = query.from_user.id
                is_verified = await db.is_user_verified(user_id)
                if not is_verified:
                    # Redirect to verification
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start=file_{user_id}_{file_id}")
                    return

            # In PM, send the file directly (only if verified or verification disabled)
    ...
    elif query.data.startswith("filep"):
        ident, file_id = query.data.split("#")
        user = query.message.reply_to_message.from_user.id if query.message.reply_to_message else query.from_user.id
        if int(user) != 0 and query.from_user.id != int(user):
            return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
        ...
        if query.message.chat.type == enums.ChatType.PRIVATE:
            ...
            if IS_VERIFY:
                ...
                if not is_verified:
                    # Redirect to verification
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start=filep_{user_id}_{file_id}")
                    return
            ...
            # In groups, redirect to PM
            await query.answer(url=f"https://t.me/{temp.U_NAME}?start=filep_{query.message.chat.id}_{file_id}")
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴇɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪꜱ !", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴇɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        try:
            btn = [[ 
                ...
                InlineKeyboardButton('🏠 ᴍᴇɴᴜ', callback_data='start')
            ]]
            reply_markup = InlineKeyboardMarkup(btn)
            await client.edit_message_media(                
                chat_id=query.message.chat.id, 
        ...
        try:
            ...
            if has_free_trial:
                ...
                await query.answer("✅ Free trial activated!")
                return    
        except Exception as e:
            print(e)
            await query.answer("❌ Error activating free trial. Please try again.", show_alert=True)
        ...
        try:
            btn = [[
                InlineKeyboardButton('💳 ᴜᴘɪ ᴘᴀʏᴍᴇɴᴛ', callback_data='upi_info'),
                InlineKeyboardButton('⭐ ꜱᴛᴀʀ ᴘᴀʏᴍᴇɴᴛ ⭐', callback_data='star_info'),
            ],[
        ...
        try:
            btn = [[
                InlineKeyboardButton('💳 ᴜᴘɪ ᴘᴀʏᴍᴇɴᴛ', callback_data='upi_donation'),
                InlineKeyboardButton('📱 ǫʀ ᴄᴏᴅᴇ', callback_data='qr_donation')
            ],[
    ...
    elif query.data == "upi_donation":
        try:
            btn = [[
                InlineKeyboardButton('📞 ᴄᴏɴᴛᴀᴄᴛ ᴏᴡɴᴇʀ', url=OWNER_LNK),
                InlineKeyboardButton('🔙 ʙᴀᴄᴋ', callback_data='donation')
            ]]
            await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("⚜️ 𝐑𝐞𝐦𝐨𝐯𝐞 𝐚ｄｓ ⚜️", url=f"https://t.me/{temp.U_NAME}?start=premium"),
                InlineKeyboardButton("Sᴇɴᴅ Aʟʟ", callback_data=f"sendfiles#{key}")
            ])
